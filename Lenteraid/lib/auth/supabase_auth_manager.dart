  ...
  Future<app_user.User?> signInWithGoogle(BuildContext context) async {
    try {
      final response = await SupabaseConfig.auth.signInWithOAuth(
        ...
        redirectTo: kIsWeb 
            ? null 
            : 'io.supabase.lenteradiamflow://login-callback', // Fallback for mobile
      );

      // Note: On web, this will redirect the entire page.
      // After redirect, the user will be handled by the onAuthStateChange listener in main.dart
      return null; 
    } catch (e) {
