...
#from local_tts_service import get_local_tts_service  # Local TTS instead of Edge TTS
...
async def startup_event():
    ...
    #     logger.info("✓ Local TTS (SpeechT5) initialized - Fully offline")
    # except Exception as e:
    #     logger.error(f"✗ Failed to initialize Local TTS: {e}")
    
    # Check AI Service (Ollama/OpenAI)
    try:
        if hasattr(ai_service, 'ollama'):
            is_healthy = await ai_service.ollama.check_health()
            if is_healthy:
...
async def health_check():
    ...
    # Check AI service (Ollama or OpenAI)
    ai_healthy = False
    if hasattr(ai_service, 'ollama'):
        ai_healthy = await ai_service.ollama.check_health()
    else:
        ai_healthy = True  # OpenAI mode
    ...
    return {
        "status": "ok" if all([ai_healthy, whisper_healthy, tts_healthy]) else "degraded",
        "services": {
            "ai": "ready" if ai_healthy else "unavailable",
            "whisper": "ready" if whisper_healthy else "unavailable",
            "tts": "ready" if tts_healthy else "unavailable"
        },
        "info": {
            "ai_mode": "ollama" if hasattr(ai_service, 'ollama') else "openai",
